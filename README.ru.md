...
```bash
python scripts/obsidian_vault_bootstrap.py --vault "~/Documents/Obsidian Vault"
```
- добавить рекомендуемые community plugins в `.obsidian/community-plugins.json`:
```bash
python scripts/obsidian_vault_bootstrap.py --vault "~/Documents/Obsidian Vault" --with-plugins
```
- построить индекс заметок (frontmatter + links) для проверки структуры:
```bash
python scripts/obsidian_vault_bootstrap.py --vault "~/Documents/Obsidian Vault" --index
```
