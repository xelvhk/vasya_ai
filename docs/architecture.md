...
- `apps/api/routes/memory.py` exposes `/v1/memory/status`, `/v1/memory/recent`, `/v1/memory/search`, `/v1/memory/digest`, `/v1/memory/digests` (with optional `date_from`/`date_to`), and `/v1/memory/sync` for desktop/mobile clients
